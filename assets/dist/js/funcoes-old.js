$(document).ready(function(){
    $('#dbfile').change(function(){
        ...
        var ext = $('#dbfile').val().split('.').pop().toLowerCase();
if($.inArray(ext, ['dbf']) == -1) {
    alert('Extensão Invalida!!');
}
});
 $('#ifcile').change(function(){
var ext = $('#ifcile').val().split('.').pop().toLowerCase();
if($.inArray(ext, ['ifc']) == -1) {
    alert('Extensão Invalida!!');
}
});
 $('#fbxile').change(function(){
var ext = $('#fbxile').val().split('.').pop().toLowerCase();
if($.inArray(ext, ['fbx']) == -1) {
    alert('Extensão Invalida!!');
}
});
    ...
    jQuery("#form-etapa-edita").submit(function(e){
        ...
        if (obraID != '' && codigoEtapa != '' && peso != '' && etapaID != '') {
            jQuery.ajax({
             ...
             url: Basepath + "/saas/etapas/gravarEdicao",
                dataType: "html",
                success: function(result){
                    if (result.substring(0,7) == 'sucesso') {
                        $('#tipoSuccess').removeClass('hidden');
    ...
    jQuery("#form-obra-edita").submit(function(e){
        ...
        if (nome != '' && cidade != '' && endereco != '' && clienteID != '') {
            jQuery.ajax({
               ...
               url: Basepath + "/saas/obras/gravarEdicao",
                dataType: "html",
                success: function(result){
                    if (result.substring(0,7) == 'sucesso') {
                        $('#tipoSuccess').removeClass('hidden');
